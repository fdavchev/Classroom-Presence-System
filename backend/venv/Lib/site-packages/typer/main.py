...
class Typer:
    ...
    def __init__(
        ...
        *,
        ...
        cls: Annotated[
            ...
            Doc(
                """
                The class of this  Mainly used when [using the Click library underneath](https://typer.tiangolo.com/tutorial/using-click/). Can usually be left at the default value `None`.
        ...
        callback: Annotated[
            ...
            Doc(
                """
                Add a callback to the main Typer  Can be overridden with `@callback()`.
        ...
        add_completion: Annotated[
            ...
            Doc(
                """
                Toggle whether or not to add the `--install-completion` and `--show-completion` options to the 
    ...
    def callback(
        ...
        *,
        ...
        """
        Using the decorator `@callback`, you can declare the CLI parameters for the main CLI application.
    ...
    def command(
        ...
        *,
        ...
        """
        Using the decorator `@command`, you can define a subcommand of the previously defined Typer 
    ...
    def add_typer(
        ...
        *,
        ...
        """
        Add subcommands to the main app using `add_typer()`.
...
def solve_typer_info_help(typer_info: TyperInfo) -> str:
    # Priority 1: Explicit value was set in add_typer()
    if not isinstance(typer_info.help, DefaultPlaceholder):
        return inspect.cleandoc(typer_info.help or "")
    # Priority 2: Explicit value was set in sub_callback()
    ...
    # Priority 4: Implicit inference from callback docstring in add_typer()
    if typer_info.callback:
        doc = inspect.getdoc(typer_info.callback)
        if doc:
            return doc
    # Priority 5: Implicit inference from callback docstring in @callback()
    if typer_info.typer_instance and typer_info.typer_instance.registered_callback:
...
def solve_typer_info_defaults(typer_info: TyperInfo) -> TyperInfo:
    ...
    for name, value in typer_info.__dict__.items():
        # Priority 1: Value was set in add_typer()
        if not isinstance(value, DefaultPlaceholder):
            values[name] = value
            continue
        # Priority 2: Value was set in @subcallback()
        try:
