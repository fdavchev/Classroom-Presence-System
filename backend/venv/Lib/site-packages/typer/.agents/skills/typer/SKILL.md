...
@command(help="[bold red]Delete[/bold red] a user with [italic]USERNAME[/italic].")
def delete(
    username: Annotated[
...
To disable Rich formatting, set `rich_markup_mode` to `None` when creating a `Typer()`  By default (when no value is given), Rich formatting is enabled.
...
@command(help="**Delete** a user with *USERNAME*.")
def delete(
