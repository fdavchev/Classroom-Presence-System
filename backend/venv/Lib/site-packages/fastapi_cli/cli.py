...
try:
    ...
    add_typer(fastapi_new_cli)  # pragma: no cover
except ImportError:  # pragma: no cover
    pass
...
def callback(
    ...
    """
    FastAPI CLI - The [bold]fastapi[/bold] command line  😎
...
def dev(
    path: Annotated[
        Path | None,
        typer.Argument(
            help="A path to a Python file or package directory (with [blue]__init__.py[/blue] files) containing a [bold]FastAPI[/bold]  If not provided, a default set of paths will be tried."
        ),
    ] = None,
    *,
    host: Annotated[
    ...
    port: Annotated[
        int,
        typer.Option(
            help="The port to serve on. You would normally have a termination proxy on top (another program) handling HTTPS on port [blue]443[/blue] and HTTP on port [blue]80[/blue], transferring the communication to your ",
            envvar="PORT",
        ),
    ] = 8000,
    reload: Annotated[
